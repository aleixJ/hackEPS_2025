"""Clean mobility statistics for LA: essential functions only.

Provides:
- batch Overpass fetch and tile assignment
- per-category and total matrices
- min-max normalization and crime-like JSON output
"""
...
# Category definitions -> list of (key, value) tag pairs to match
CATEGORY_TAGS = {
...
def mobility_counts_by_parcels(nx: int, ny: int, *, overpass_url: str = DEFAULT_OVERPASS) -> List[Dict[str, Any]]:
    """Batch-fetch Overpass for full LA bbox and assign elements to tiles.

    Returns list of parcels with keys: i, j, bounds, center, counts, total.
    """
    if nx <= 0 or ny <= 0:
    ...
    # tile indexed as [j][i] where j=0 south
...
def mobility_matrix_json(nx: int, ny: int, *, overpass_url: str = DEFAULT_OVERPASS) -> Dict[str, Any]:
    parcels = mobility_counts_by_parcels(nx, ny, overpass_url=overpass_url)
...
def mobility_crimelike_json(nx: int, ny: int, *, overpass_url: str = DEFAULT_OVERPASS) -> List[Dict[str, Any]]:
    obj = mobility_matrix_json(nx, ny, overpass_url=overpass_url)
...
def save_mobility_crimelike_json(data: List[Dict[str, Any]], filename: Optional[str] = None) -> str:
    base_dir = os.path.join(os.path.dirname(__file__), "jsons")
    os.makedirs(base_dir, exist_ok=True)
    if not filename:
        stamp = time.strftime("%Y%m%dT%H%M%SZ", time.gmtime())
        filename = f"crime_matrix_{stamp}.json"
...
def save_mobility_matrix_json(obj: Dict[str, Any], filename: Optional[str] = None) -> str:
    base_dir = os.path.join(os.path.dirname(__file__), "jsons")
    os.makedirs(base_dir, exist_ok=True)
    if not filename:
        stamp = time.strftime("%Y%m%dT%H%M%SZ", time.gmtime())
        filename = f"mobility_matrix_{stamp}.json"
    path = os.path.join(base_dir, filename)
    with open(path, "w", encoding="utf-8") as fh:
...
if __name__ == "__main__":
    # default: generate and save a 20x20 crime-like normalized JSON
    nx = 20
    ny = 20
    data = mobility_crimelike_json(nx, ny)
    out = save_mobility_crimelike_json(data)
    print(f"Saved crime-like mobility JSON to: {out}")